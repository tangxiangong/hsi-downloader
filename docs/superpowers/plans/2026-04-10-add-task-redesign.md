# AddTaskDialog 改造 + 分块进度条 Bug 修复 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Redesign AddTaskDialog with HTTP/BT tab switching, per-task configuration (priority, speed limit, checksum, headers), and fix the chunk progress bar not showing on initial download.

**Architecture:** Three independent changes: (1) backend `AddTaskOptions` adds `headers` field, (2) frontend `AddTaskDialog` is rewritten with tab UI and layered config, (3) backend progress listener forwards `Initialized` event to fix chunk progress bar.

**Tech Stack:** Rust (hsi-core), SolidJS + TypeScript (src-ui), TailwindCSS + DaisyUI

---

### Task 1: Backend — Add `headers` field to `AddTaskOptions`

**Files:**
- Modify: `hsi-core/src/types.rs:293-303`
- Modify: `hsi-core/src/downloader.rs:937-972`

- [ ] **Step 1: Add `headers` field to `AddTaskOptions`**

In `hsi-core/src/types.rs`, add `headers` after `selected_files`:
...
- [ ] **Step 2: Pass `headers` to Task in `add_task_with_options`**

In `hsi-core/src/downloader.rs`, change line 963 from:
...
Run: `cargo check -p hsi-core`
Expected: compiles without errors.

- [ ] **Step 4: Commit**

```bash
git add hsi-core/src/types.rs hsi-core/src/downloader.rs
git commit -m "feat: add headers field to AddTaskOptions"
```

---

### Task 2: Fix chunk progress bar not showing on initial download

**Files:**
- Modify: `hsi-core/src/downloader.rs:1080-1097`

- [ ] **Step 1: Forward Initialized event to queue event channel**

In `hsi-core/src/downloader.rs`, find the `ProgressEvent::Initialized` handler in the progress listener (around line 1080-1097). After the existing block that updates `task.chunk_progress` (ends at line 1097), add the event forwarding:
...
Replace with (add forwarding after the existing block):
```rust
                        ProgressEvent::Initialized {
                            total_size, chunks, ..
                        } => {
...
```bash
git add hsi-core/src/downloader.rs
git commit -m "fix: forward Initialized event so chunk progress bar shows on first download"
```

---

### Task 3: Frontend — Update types and commands for new AddTaskOptions fields

**Files:**
- Modify: `src-ui/src/lib/types.ts:87-95`

- [ ] **Step 1: Add `headers` to frontend `AddTaskOptions` type**

In `src-ui/src/lib/types.ts`, update the `AddTaskOptions` interface:
...
git commit -m "feat: add headers field to frontend AddTaskOptions type"
```

---

### Task 4: Frontend — Rewrite AddTaskDialog with Tab switching and per-task config

**Files:**
- Modify: `src-ui/src/components/AddTaskDialog.tsx`

- [ ] **Step 1: Rewrite AddTaskDialog with complete new implementation**

Replace the entire content of `src-ui/src/components/AddTaskDialog.tsx` with:
...
```bash
git add src-ui/src/components/AddTaskDialog.tsx
git commit -m "feat: redesign AddTaskDialog with tab switching and per-task config"
```
